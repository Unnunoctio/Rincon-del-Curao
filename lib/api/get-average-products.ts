...
export const getAverageProducts = async (availableWebs: string[] = []): Promise<ProductPreview[]> => {
  ...
  const filtered = products
    ...
    .filter((product: any) => generateAverage(product.websites) > 0)

  filtered.sort((a: any, b: any) => {
    const averA = generateAverage(a.websites)
    const averB = generateAverage(b.websites)

    if (averA !== averB) return averB - averA
