...
import { AlcoholicIcon, BitternessIcon, BrandIcon, CategoryIcon, ContentIcon, MadeInIcon, PackageIcon, QuantityIcon, StrainIcon, VarietyIcon, VineyardIcon } from './icons'

interface Props extends Omit<ProductUnit, 'name' | 'category'> {
  quantity: number
}

export const FeatureList: React.FC<Props> = ({ brand, subCategory, quantity, content, variety, strain, vineyard, alcoholicGrade, bitterness, package: packageProp, madeIn }) => {
  return (
    <div className='mt-6'>
      <h3 className='text-primary font-medium text-[20px]'>Caracteristicas</h3>
      <div className='grid grid-cols-1 xs:grid-cols-2 gap-x-3 mt-2'>
        ...
        {vineyard !== null && (
          <FeatureItem title='Viña' value={vineyard}>
