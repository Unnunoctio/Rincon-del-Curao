        ...
        <li key={index}>
          ...
          <ul className='flex flex-col gap-4 pl-4 border-l border-primary'>
            {link.categories.map((category, index) => (
              <li key={index}>
                <Link href={`${link.route}?sub_category=${category.query}`} onClick={onClose} aria-label={category.name.toLowerCase()} className='text-secondary hover:underline'>
                  {category.name}
                </Link>
              </li>
            ))}
            <li>
              <Link href={link.route} onClick={onClose} aria-label='ver todos' className='text-active hover:underline'>
                Ver Todos
              </Link>
            </li>
          </ul>
